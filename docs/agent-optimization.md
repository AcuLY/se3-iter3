...
- Agent 流式运行的错误事件会在右侧对话中显示服务端返回的用户可读原因，并保持画布不变，不再误套用本地 fallback 行程更新。
...
- 扩展可替换的 Agent 框架适配层，在保持当前 DeepSeek tool calls 能力的前提下接入更多模型或编排运行时。
